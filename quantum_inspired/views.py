...
class RunSimulationView(APIView):
    ...
    def post(self, request):
        ...
        # Run the selected simulation synchronously (quick, deterministic heuristics)
        result = {}
        try:
            if technique == 'grover':
                dataset = params.get('dataset', [])
                pred = lambda x: params.get('predicate', lambda v: False)
                result = grover_search_simulation(dataset, lambda v: True if v in params.get('matches', []) else False)
                ...
                # energy_fn is not serializable; we accept a simple key to choose best
